...
# Read word frequencies and headline counts from CSV file
word_freq = {}
headline_counts = {}
with open('word_info_dataset2.csv', 'r') as file:
	...
	for row in reader:
		...
		count = int(row[2])
		word_freq[word] = freq
		headline_counts[word] = count

# Normalize word frequencies based on headline counts
normalized_freq = {}
for word, freq in word_freq.items():
	count = headline_counts[word]
	normalized_freq[word] = freq / count

# Sort normalized word frequencies in descending order
...
# Now you can use the 'dataset2_top_words.json' file to create an interactive D3 graph